...
/**
 * Измененный {@link EnumSerializer} для модификации {@link CustomEnumSerializer#serialize(Enum, JsonGenerator, SerializerProvider)}:
 * необходимо сериализовать значение enum без обрамляющих двойных кавычек.
 * Оставлен в java, чтобы изменения были прозрачны.
 */
@JacksonStdImpl
    ...
    implements ContextualSerializer
{
    ...
    public CustomEnumSerializer(EnumValues v, Boolean serializeAsIndex)
    {
    ...
    public static CustomEnumSerializer construct(Class<?> enumClass, SerializationConfig config,
                                                 BeanDescription beanDesc, JsonFormat.Value format)
    {
        ...
        return new CustomEnumSerializer(v, serializeAsIndex);
    }
            ...
            BeanProperty property) throws JsonMappingException
    {
        ...
        if (format != null) {
            ...
            if (!Objects.equals(serializeAsIndex, _serializeAsIndex)) {
                return new CustomEnumSerializer(_values, serializeAsIndex);
            }
        }
        return this;
    }
        ...
        throws IOException
    {
        ...
        // вместо "gen.writeString(_values.serializedValueFor(en));" пишем значение-объект как есть
//        gen.writeString(_values.serializedValueFor(en));
//        gen.writeRawValue(_values.serializedValueFor(en));
//        gen.writeObject(en);
        gen.writeEmbeddedObject(en);
    }
